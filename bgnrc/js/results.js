//Live results 
var path = 'http://code.boxfishbg.info/liveparser_hill/files/'; //basepath for result files
...
var champnames = {all:"всички"};
var typenames = {race:"Състезание", training:"Тренировки"};
...
{	
	...
	$.jsonp({
		...
		success: function(json) {
			...
			//event itinerary
			trainigSessions=json.event.stagelist.stage[0].training_sessions;
			$('#training-sessions').html(trainigSessions);
			raceSessions=json.event.stagelist.stage[1].race_sessions;
			$('#race-sessions').html(raceSessions);
...
function getresults(stage, champ, group)
{
	...
	//Get stage results
	var urlstage = path + eventdata.id + '/' + stage + '_live_' + champ + '_' + group + '.json?callback=?';
	
	$.jsonp({
		url: urlstage,
		callback: "callback",
		success: function(json) {
		   var stageresults='<h3>Резултати от '+typenames[stage]+'</h3><div class="table-responsive"><table class="table table-striped results"><thead><tr><th class="center">Поз.</th><th class="center">№</th><th>Пилот</th><th>Автомобил</th><th>Клас</th><th class="center">Време</th><th class="center">Разлика</th><th class="center">Манш 1</th><th class="center">Манш 2</th><th class="center">Манш 3</th></tr></thead><tbody>';
			
			if ($.isEmptyObject(json)) {
				stageresults='<h3>Резултати от '+typenames[stage]+'</h3><div>Все още няма данни за етапа.</div>';
			  ...
			  if (json.result.length) {
				$.each(json.result, function(key, val){
                    ...
                    if (typeof val.stagetime==="string") {var totaltime = val.stagetime;} else {var totaltime="-:-";}
                    if (val.state=="DNF") {val.position="DNF"; }
					stageresults = stageresults + '<tr><td class="center">' + val.position + '</td><td class="center">' + val.number + '</td><td class="crew"><img src="images/' + val.drivernat + '.gif" alt="flag" />&nbsp;' + val.driver + '</td><td>' + val.car + '</td><td>' + val.class + '</td><td class="center">' + totaltime + '</td><td class="center">' + val.difffirst + '</td><td class="center">' + val.session1 + '</td><td class="center">' + val.session2 + '</td><td class="center">' + val.session3 + '</td></tr>';
				})
			  }
			  
			  else {
                 if (typeof json.result.stagetime==="string") {var totaltime = json.result.stagetime;} else {var totaltime="-:-";}
                 stageresults = stageresults + '<tr><td class="center">' + json.result.position + '</td><td class="center">' + json.result.number + '</td><td class="crew"><img src="images/' + json.result.drivernat + '.gif" alt="flag" />&nbsp;' + json.result.driver + '</td><td>' + json.result.car + '</td><td>' + json.result.class + '</td><td class="center">' + totaltime + '</td><td></td><td class="center">' + json.result.session1 + '</td><td class="center">' + json.result.session2 + '</td><td class="center">' + json.result.session3 + '</td></tr>';
			  }
...
{
	if(time.substr(0, 10)<=eventdata.start) {
		stage="training";
	}
	
	if(time.substr(0, 10)>=eventdata.end) {
		stage="race";
	}
...
function getentries()
{
	...
	$.jsonp({
		...
		callback: "callback",
		success: function(json) {
		   var entries='<table class="table table-striped table-responsive"><thead><tr><th>№</th><th>Отбор</th><th>Пилот</th><th>Автомобил</th><th>Клас</th></tr></thead><tbody>';
			  $.each(json.entry, function(key, val){
				entries = entries + '<tr><td>' + val.number + '</td><td>' + val.entrant + '</td><td>' + val.driver + '</td><td>' + val.car + '</td><td>' + val.class + '</td></tr>';
		...
		success: function(json) {
		   ...
		   if ($.isEmptyObject(json)) {
				...
				if (json.flash.length) {
					...
					//flashnews = flashnews + '<div class="flashtime">' + json.flash.time + '</div><div class="flashplace">' + json.flash.place + '</div><div class="flashdesc">' + json.flash.description + '</div>';
...
function loadRetired() 
{
	...
	$.jsonp({
		...
		callback: "callback",
		success: function(json) {
		   var retired='<table class="table table-striped table-responsive"><thead><tr><th>№</th><th>Отбор</th><th>Пилот</th><th>Автомобил</th><th>Място</th><th>Причина</th></tr></thead><tbody>';
		   ...
		   if ($.isEmptyObject(json)) {
				...
				if (json.retire.length) {
					$.each(json.retire, function(key, val){
						retired = retired + '<tr><td>' + val.number + '</td><td>' + evententries[val.number].entrant + '</td><td>' + evententries[val.number].driver + '</td><td>' + evententries[val.number].car + '</td><td>' + val.location + '</td><td>' + val.reason + '</td></tr>';
					})
				  }
				  
				  else {
					retired = retired + '<tr><td>' + json.retire.number + '</td><td>' + evententries[json.retire.number].entrant + '</td><td>' + evententries[json.retire.number].driver + '</td><td>' + evententries[json.retire.number].car + '</td><td>' + json.retire.location + '</td><td>' + json.retire.reason + '</td></tr>';
				  }
